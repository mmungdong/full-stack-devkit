...
shop-app 是一个基于 Go 语言开发的现代化微服务应用，采用简洁架构设计，具有代码质量高、扩展能力强、符合 Go 编码及最佳实践等特点。

shop-app 具有以下特性：
- 软件架构：采用简洁架构设计，确保项目结构清晰、易维护；
- 高频 Go 包：使用了 Go 项目开发中常用的包，如 gin、gorm、gin、uuid、cobra、viper、pflag、resty、govalidator、slog、protobuf、casbin、onexstack 等；
...
├── linux/  
│   └── amd64/  
│       └── shop-apiserver  # apiserver 服务二进制文件  
└── darwin/  
    └── amd64/  
        └── shop-apiserver  
```
...
```bash  
# 启动 apiserver 服务  
$ _output/platforms/linux/amd64/shop-apiserver --config configs/shop-apiserver.yaml  
# 服务将在以下端口启动：  
...
# 构建镜像  
$ make image
$ docker run --name shop-apiserver -v configs/shop-apiserver.yaml:/etc/shop-apiserver.yaml -p 5555:5555 docker.io/mungdong/shop-apiserver:latest -c /etc/shop-apiserver.yaml
```

**配置文件示例：**  

shop-apiserver 配置文件 `configs/shop-apiserver.yaml`：
...
<!-- 这里会自动显示贡献者头像 -->
<a href="github.com/onexstack/shop-app/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=github.com/onexstack/shop-app" />
</a>
...
- [项目文档](docs/)
- [问题追踪](github.com/onexstack/shop-app/issues)
- [讨论区](github.com/onexstack/shop-app/discussions)
- [项目看板](github.com/onexstack/shop-app/projects)
- [发布页面](github.com/onexstack/shop-app/releases)

### 支持

如果这个项目对您有帮助，请考虑给我们一个 ⭐️ 来支持项目发展！

[![Star History Chart](https://api.star-history.com/svg?repos=github.com/onexstack/shop-app&type=Date)](https://star-history.com/#github.com/onexstack/shop-app&Date)
